    ...
    { g_Texts[lang][MTK_ROW_DESIGN_CAP_CHANGED], g_Texts[lang][MTK_HINT_DESIGN_CAP_CHANGED] },
     ...
     /*   int fullContentHeight =
        ...
        // Header band (no scroll offset for header/meter)
        {
            ...
            CString btnLabel;
            if (!lang) {
                btnLabel = L"Export Report";   //Export PDF
           }
            else {
                btnLabel = L"レポートのエクスポート";
            }
        ...
        // --- Score legend strip ---
        {
            ...
            if (!lang) {
                manipulated= L"Likely Manipulated  0–39";
                suspecious= L"Suspicious  40–69";
                genuine= L"Genuine  70–100";

            }
            else {
                manipulated = L"操作された可能性が高い 0-39";
                suspecious = L"疑わしい  40–69";
                genuine = L"本物  70–100";
            }

            struct LegendSeg { COLORREF col; const wchar_t* label; } segs[] = {
                { RGB(210,60,60),    manipulated },
                { RGB(240,170,40),   suspecious },
                { RGB(0,160,80),     genuine },
        ...
        {
            CString footnote = (lang)
                ? L"* 情報はデバイスが生成したバッテリーレポートから取得されています。"
                L"結果はデバイスのファームウェアが提供するデータによって異なる場合があります。"
                : L"* Information is sourced from the battery report generated by your device. "
...

    {
        ...
        if (!f.Open(htmlTemp, CFile::modeCreate | CFile::modeWrite))
        {
            if (eng_lang) {
                ...
                AfxMessageBox(L"一時的なHTMLファイルの書き込みに失敗しました。",
                    MB_ICONERROR | MB_TOPMOST);
                return false;
            }
        }
...
CString CManipulationDlg::BuildHtmlReport() const
{
    ...
    // ── Strings switched by language ──────────────────────────────────────
    CString strNotTriggered = eng_lang ? L"&#8212; not triggered &#8212;"
        : L"&#8212; 該当なし &#8212;";
    CString strReportTitle = eng_lang ? L"Battery Manipulation Detection Report"
        : L"バッテリー改ざん検出レポート";
    CString strGenerated = eng_lang ? L"Generated"
        : L"生成日時";
    CString strVerdict = eng_lang ? L"Verdict"
        : L"判定";
    CString strLegendLow = eng_lang ? L"Likely Manipulated (0&#8211;39)"
        : L"改ざんの可能性が高い (0&#8211;39)";
    CString strLegendMid = eng_lang ? L"Suspicious (40&#8211;69)"
        : L"疑わしい (40&#8211;69)";
    CString strLegendHigh = eng_lang ? L"Genuine (70&#8211;100)"
        : L"正規品 (70&#8211;100)";
    CString strColIndicator = eng_lang ? L"Indicator"
        : L"指標";
    CString strColDetail = eng_lang ? L"Detail / Evidence"
        : L"詳細 / 根拠";
    CString strFootnote = eng_lang
        ? L"* Information is sourced from the battery report generated "
        L"by your device. Results may vary depending on the data provided by your device&#39;s firmware."
        : L"* 情報はデバイスが生成したバッテリーレポートから取得されています。"
        L"結果はデバイスのファームウェアが提供するデータによって異なる場合があります。";
    ...
    html.Format(
        ...
        L"  </div>\n"
        L"  <div class='body'>\n"
        L"    <div class='verdict'>%s: "
        L"      <span style='color:%s;'>%s</span>"
        L"      <span style='font-size:15px;font-weight:normal;color:#888;'>&nbsp;(%d / 100)</span>"
        L"    </div>\n"
        L"    %s\n"
        L"    <div class='legend'>\n"
        L"      <span class='leg'><span class='sw' style='background:#d23c3c'></span>%s</span>\n"
        L"      <span class='leg'><span class='sw' style='background:#f0aa28'></span>%s</span>\n"
        L"      <span class='leg'><span class='sw' style='background:#00a050'></span>%s</span>\n"
        L"    </div>\n"
        L"    <table>\n"
        L"      <thead><tr>"
        L"        <th style='width:32px'></th>"
        ...
        L"      <tbody>\n%s\n      </tbody>\n"
        L"    </table>\n"
        L"    <p class='footer'>%s</p>\n"
        L"  </div>\n"
        L"</div></body></html>\n",
        strReportTitle.GetString(),   // <title>
        bandColor.GetString(),        // header background
        strReportTitle.GetString(),   // <h1>
        strGenerated.GetString(),     // "Generated"
        stamp.GetString(),            // timestamp
        strVerdict.GetString(),       // "Verdict"
        bandColor.GetString(),        // verdict colour
        r.status.GetString(),         // verdict text
        r.score,                      // score number
        meterBar.GetString(),         // meter HTML
        strLegendLow.GetString(),     // legend low
        strLegendMid.GetString(),     // legend mid
        strLegendHigh.GetString(),    // legend high
        strColIndicator.GetString(),  // table col 1
        strColDetail.GetString(),     // table col 2
        tableRows.GetString(),        // all rows
        strFootnote.GetString()       // footer
    );
...
void CManipulationDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
    if (s_exportBtnClientRect.PtInRect(point))
    {
        ...
        tdc.pszWindowTitle = eng_lang
            ? L"Export Report"
            : L"レポートをエクスポート";

        tdc.pszMainInstruction = eng_lang
            ? L"Choose export format"
            : L"エクスポート形式を選択してください";

        TASKDIALOG_BUTTON buttonsEN[] = {
            { 100, L"Export HTML\nOpens in browser — use Ctrl+P to save as PDF" },
            { 101, L"Export PDF\nDirect PDF via headless browser"               },
            { 102, L"Cancel\nDo not export"                                     },
        };
        TASKDIALOG_BUTTON buttonsJP[] = {
            { 100, L"HTMLでエクスポート\nブラウザで開きます — Ctrl+Pを押してPDFとして保存" },
            { 101, L"PDFでエクスポート\nヘッドレスブラウザで直接PDFを生成"               },
            { 102, L"キャンセル\nエクスポートしない"                                     },
        };

        tdc.pButtons = eng_lang ? buttonsEN : buttonsJP;
        tdc.cButtons = _countof(buttonsEN);
        tdc.nDefaultButton = 100;
        ...
        if (SUCCEEDED(TaskDialogIndirect(&tdc, &nButton, nullptr, nullptr)))
        {
            ...
            // nButton == 102 or IDCANCEL — do nothing
        }
    }
    CDialogEx::OnLButtonUp(nFlags, point);
}