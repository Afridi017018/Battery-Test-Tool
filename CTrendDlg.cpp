...
BOOL CTrendDlg::OnInitDialog() {
    ...
    const CString reportPath = GenBatteryReportPath();
    RunPowerCfgBatteryReport(reportPath); // best effort
    LoadBatteryCapacityHistory(reportPath);  // NEW
...
bool CTrendDlg::LoadBatteryCapacityHistory(const CString& filePath)
{
...
bool CTrendDlg::LoadFromBatteryReport(const CString& filePath)
{
    ...
    for (std::wsregex_iterator it(tableHtml.begin(), tableHtml.end(), reRow), end; it != end; ++it) {
        ...
        float designActive = 0.f;

        // === Mathematical Formula ===
        // DesignActive = FullActive × (DesignCapacity / FullCapacity)

        size_t idx = m_full.size();
            ...
            idx < m_designCapacity.size())
        {
            ...
            if (fullCap > 0.f) {
designActive = fullActive * (designCap / fullCap);
//CString msg;
