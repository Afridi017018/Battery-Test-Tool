...
static const wchar_t* g_Texts[2][TK_COUNT] =
{
    // English
    {
        ...
        L"No valid Connected Standby data for chart (glitched rows skipped).",         // TK_NO_VALID_STANDBY
        L"Connected Standby Battery Life Trend (hours)",                               // TK_TOP_TITLE
        L"Hours",                                                                      // TK_TOP_Y_LABEL
        L"Standby (Full Charge)",                                                      // TK_LEGEND_FULL
        L"Standby (Design Capacity)",                                                  // TK_LEGEND_DESIGN
    ...
    // Japanese
    {
        ...
        L"グラフ用の有効な接続スタンバイデータがありません（異常値の行は除外されました）。",   // TK_NO_VALID_STANDBY
        L"接続スタンバイ時のバッテリー寿命の傾向（時間）",                                      // TK_TOP_TITLE
        L"時間",                                                                         // TK_TOP_Y_LABEL
        L"スタンバイ（フル充電）",                                                         // TK_LEGEND_FULL
        L"スタンバイ（設計容量）",                                                         // TK_LEGEND_DESIGN
        L"期間",                                                                         // TK_AXIS_PERIOD
...
void StandByDlg::DoDataExchange(CDataExchange* pDX) { CDialogEx::DoDataExchange(pDX); }

BEGIN_MESSAGE_MAP(StandByDlg, CDialogEx)
    ON_WM_PAINT()
    ON_WM_DESTROY()
    ON_WM_SIZE()
    ON_WM_ERASEBKGND()
...
void StandByDlg::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
...
BOOL StandByDlg::OnInitDialog() {
    ...
    if (eng_lang)
    {
        SetWindowTextW(L"Standby Battery Life Trend");
    }
    else
    {
        SetWindowTextW(L"スタンバイ時のバッテリー寿命の傾向");
    }

    return TRUE;
}

void StandByDlg::OnDestroy() {
    CDialogEx::OnDestroy();
    if (m_gdiplusToken) GdiplusShutdown(m_gdiplusToken);
}

BOOL StandByDlg::OnEraseBkgnd(CDC*) { return TRUE; }
void StandByDlg::OnSize(UINT nType, int cx, int cy) {
    CDialogEx::OnSize(nType, cx, cy);
    if (IsWindowVisible()) Invalidate(FALSE);
}

void StandByDlg::SetData(const std::vector<CString>& p,
...
//
// NOW parsing the CONNECTED STANDBY columns:
//   - At Full Charge   -> times[1]
...
bool StandByDlg::LoadFromBatteryReport(const CString& filePath)
{
    ...
    for (std::wsregex_iterator it(tableHtml.begin(), tableHtml.end(), reRow), end; it != end; ++it) {
        ...
        // Extract times from the WHOLE row, then pick standby columns
        std::wstring rowText;
        ...
        m_full.push_back(fullStandby);
        m_design.push_back(designStandby);
    }
...
// =================== DRAWING (same charts, with padding) ===================
void StandByDlg::OnPaint()
{
    CPaintDC dc(this);
    ...
    // ---------- TOP CHART: Connected Standby (Full vs Design) ----------
    ...
    if (w1 > 10.f && h1 > 10.f) {
        ...
        // Standby-friendly filter
        const float MAX_STANDBY_HOURS = 1500.f;
        ...
        for (size_t i = 0; i < m_full.size() && i < m_design.size() && i < m_periods.size(); ++i) {
            const float sf = (float)m_full[i];
            const float sd = (float)m_design[i];
            if (sf >= 0.f && sf <= MAX_STANDBY_HOURS &&
                sd >= 0.f && sd <= MAX_STANDBY_HOURS) {
                fPeriods.push_back(m_periods[i]);
                fFull.push_back((Gdiplus::REAL)sf);
                fDesign.push_back((Gdiplus::REAL)sd);
            }
        }
        ...
        if (useF < 2) {
            ...
            g.DrawString(g_Texts[lang][TK_NO_VALID_STANDBY],
            ...
            // Legend
            {
                ...
                float lx = x1 + w1 - 220.f, ly = y1 + 10.f;
